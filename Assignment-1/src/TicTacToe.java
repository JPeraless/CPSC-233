...
public class TicTacToe extends Canvas {
     ...
     *
     * @param rows The number of rows that the Array will have.
     * @param columns The number of columns that the Array will have.
     * @return 2d int Array with rows as its first dimension and columns as its second dimension.
     */
    ...
    /**
     * Gets the number of rows in a given 2d Array.
     *
     * @param board The 2d int array of size [rows][columns].
     * @return The length of the board as an int.
     */
    ...
    /**
     * Gets the number of columns in a given 2d Array.
     *
     * @param board The 2d int array of size [rows][columns].
     * @return The length of the first row of the board as an int.
     */
    ...
    /**
     * Determines if a given position in the board is empty or not.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param row The first dimension index of the position to evaluate.
     * @param column The second dimension index of the position to evaluate.
     * @return True if the position in the board is 0, false otherwise.
     */
    ...
    /**
     * Set the element at the given position to the value of piece.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param row The first dimension index of the position to evaluate.
     * @param column The second dimension index of the position to evaluate.
     * @param piece Provides the int value of the piece that the position must be set to.
     */
    ...
    public static boolean full(int[][] board) {
        ...
        // Return false if there is at least one EMPTY piece. Otherwise, true.

        // create numbers ArrayList
        ArrayList<Integer> numbers = new ArrayList<>();

        // every element of board to the ArrayList numbers[]
        for (int[] ints : board) {
        ...
        // loop through every value in numbers, if an empty value is found return true.
        for (int k : numbers) {
            if (k == EMPTY) {
                return false;
            }
        ...
        // return true if no empty values were found.
        return true;
    }

    /**
     * Determines if either player wins the game by having 3 consecutive pieces in the same row.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param row The row to evaluate.
     * @param piece Provides the int value of the piece to look for.
     * @return True if there are 3 consecutive pieces at the given row, false otherwise.
     */
    public static boolean winInRow(int[][] board, int row, int piece) {

        // initialize a constant for the valid index.
        final int VALID_INDEX = columnsIn(board) - 2;


        // initialize an array with the same number of entries as there are columns in the board.
        int[] values = new int[columnsIn(board)];

        // add every value in the given row of the board to values[].
        for (int i = 0; i < columnsIn(board); i++) {
            values[i] = board[row][i];
        }

        // check if there are 3 consecutive pieces and return true if that is the case.
        for (int j = 0; j < VALID_INDEX; j++) {
            if (values[j] == piece && values[j + 1] == piece && values[j + 2] == piece) {
                return true;
            }
        }

        // return false if no consecutive 3 pieces were found.
        return false;
    }

    /**
     * Determines if either player wins the game by having 3 consecutive pieces in the same column.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param column The column to evaluate.
     * @param piece Provides the int value of the piece to look for.
     * @return True if there are 3 consecutive pieces at the given column, false otherwise.
     */
    public static boolean winInColumn(int[][] board, int column, int piece) {

        // initialize a constant for the valid index.
        final int VALID_INDEX = rowsIn(board) - 2;

        // initialize an array with the same number of entries as there are rows in the board.
        int[] values = new int[rowsIn(board)];

        // add every value in the given column of the board to values[].
        for (int i = 0; i < rowsIn(board); i++) {
            values[i] = board[i][column];
        }
    ...
    /**
     * Determines if either player wins the game by having 3 consecutive pieces in the same backslash diagonal.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param piece Provides the int value of the piece to look for.
     * @return True if there are 3 consecutive pieces at the given backslash diagonal, false otherwise.
     */
    public static boolean winInDiagonalBS(int[][] board, int piece) {

        // initialize constants for the valid indices.
        final int VALID_ROWS = rowsIn(board) - 2;
        final int VALID_COLS = columnsIn(board) - 2;

        // loop through every entry in the board.
        for (int row = 0; row < rowsIn(board); row++) {
            for (int col = 0; col < columnsIn(board); col++) {
    ...
    /**
     * Determines if either player wins the game by having 3 consecutive pieces in the same forward slash diagonal.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param piece Provides the int value of the piece to look for.
     * @return True if there are 3 consecutive pieces at the given forward slash diagonal, false otherwise.
     */
    public static boolean winInDiagonalFS(int[][] board, int piece) {

        // initialize constants for the valid indices.
        final int VALID_ROWS = rowsIn(board) - 2;
        final int VALID_COLS = 1;
    ...
    /**
     * Provides hints to the human player during the game.
     *
     * @param board The 2d int array of size [rows][columns].
     * @param piece Provides the int value of the piece for which to give hints.
     * @return An array of the form {x, y}, where x is the row index and y is the column index of the position in which
     * the hint must be provided. Returns {-1, -1} by default if no hints were found.
     */
    public static int[] hint(int[][] board, int piece) {
        ...
        for (int row = 0; row < rowsIn(board); row++) {
            for (int column = 0; column < columnsIn(board); column++) {
                ...
                // if it is possible to play at board[row][column]
                if (canPlay(board, row, column)) {

                    // play
                    play(board, row, column, piece);

                    // if there exists a way to win the game.
                    if (winInRow(board, row, piece) || winInColumn(board, column, piece) || winInDiagonalBS(board, piece) || winInDiagonalFS(board, piece)) {
                        board[row][column] = EMPTY;  // remove the player's piece from the last played location
                        return new int[] {row, column};  // return the position at which the player should play to win the game, or to not lose.

                    }

                    // if there does not exist a way to win the game.
                    else {
                        board[row][column] = EMPTY;  // remove the player's piece from the last played location
                    }
                }
        ...
        // default return to indicate that no way to win or prevent a loss was found.
        return new int[]{-1, -1};
    }
