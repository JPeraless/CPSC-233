...
public class World {
    ...
    /**
     * Changes a given World object into String to provide useful output.
     *
     * @return String showing the current state of World, with a map view, and a brief description of each entity.
     */
    public String gameString() {
        // create an ArrayList to store the information about all entities
        ArrayList<String> entities = new ArrayList<String>();

        // create the map that will be printed first
        String map = worldString(this.world) + "\n";  // image of the map

        // labels of each entity
        String labels = "NAME \tS\tH\tSTATE\tINFO\n";

        for (int i = 0; i < this.entities.size(); i++) {
            entities.add(String.valueOf(this.entities.get(i)) + "\n");

        }

        String strEntities = entities.toString();

        strEntities = strEntities.replace(" ", "");  // get rid of whitespace
        strEntities = strEntities.replace(",", "");  // get rid of commas
        strEntities = strEntities.replace("[", "");  // get rid of parentheses
        strEntities = strEntities.replace("]", "");  // get rid of parentheses
    ...
    /**
     * Creates a 2D map of the world in which the entities are correctly represented.
     *
     * @param world World object to be represented as a String.
     * @return String representation of the World object inputted as a parameter.
     */
    public String worldString(Entity[][] world) {
        ...
        int rowsNo = world.length;
        int colsNo = world[0].length;
        int wallsNo = 2;

        // add a wall to every entry in the first row
        for (int i = 0; i < colsNo + wallsNo; i++) {
        ...
        row = new ArrayList<String>();  // set row to a new AL to reuse it

        for (int l = 0; l < colsNo + wallsNo; l++) {
            row.add(Character.toString(Symbol.WALL.getSymbol()));
        }
        ...
        return strCompleteWorld;
    }

    public World getLocal(int size, int row, int column) {
        // initialize World view of entity
        World worldView = new World(size, size);
